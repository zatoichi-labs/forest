...
impl<'a> ChainStore<'a> {
    ...
    pub fn messages(
        ...
        let bls_msgs: Vec<UnsignedMessage> =
            self.messages_from_cids(vec![bh.message_receipts()])?;
        let secp_msgs: Vec<SignedMessage> = self.messages_from_cids(vec![bh.message_receipts()])?;
        ...
        T: DeserializeOwned,
    {
        keys.iter()
            .map(|k| {
                ...
                let bytes = value.ok_or(Error::UndefinedKey(k.to_string()))?;
