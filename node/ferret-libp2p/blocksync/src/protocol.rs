...
pub struct CborMessage(Vec<cid::Cid>, u64, u64);
impl ser::Serialize for Message {
    fn serialize<S>(&self, serializer: S) -> Result<<S as Serializer>::Ok, <S as Serializer>::Error>
    where
        S: Serializer,
    {
        let value = CborMessage(self.start.clone(), self.request_len, self.options);
        CborMessage::serialize(&value, serializer)
    }
}
impl<'de> de::Deserialize<'de> for Message {
    ...
    where
        D: Deserializer<'de>,
    {
        let cm: CborMessage = CborMessage::deserialize(deserializer)?;
        Ok(Message {
            start: cm.0,
            request_len: cm.1,
            options: cm.2,
...
#[derive(Serialize, Deserialize)]
pub struct CborResponse(Vec<TipSetBundle>, u64, String);

#[derive(Clone, Debug)]
pub struct TipSetBundle {
    pub blocks: Vec<BlockHeader>,
    pub secp_msgs: Vec<message::UnsignedMessage>,
    pub secp_msg_includes: Vec<Vec<u64>>,

    pub bls_msgs: Vec<message::SignedMessage>,
    pub bls_msg_includes: Vec<Vec<u64>>,
}

#[derive(Serialize, Deserialize)]
pub struct CborTipsetBundle(
    Vec<BlockHeader>,
    Vec<message::UnsignedMessage>,
    Vec<Vec<u64>>,
    Vec<message::SignedMessage>,
    Vec<Vec<u64>>,
);
...
impl ser::Serialize for Response {
    ...
    where
        S: Serializer,
    {
        let value: CborResponse =
            CborResponse(self.chain.clone(), self.status, self.message.clone());
        CborResponse::serialize(&value, serializer)
    }
}
impl<'de> de::Deserialize<'de> for Response {
    ...
    where
        D: Deserializer<'de>,
    {
        let cm = CborResponse::deserialize(deserializer)?;
        Ok(Response {
            chain: cm.0,
            status: cm.1,
            message: cm.2,
...
impl ser::Serialize for TipSetBundle {
    ...
    where
        S: Serializer,
    {
        let value: CborTipsetBundle = CborTipsetBundle(
            self.blocks.clone(),
        ...
        CborTipsetBundle::serialize(&value, serializer)
    }
}
...
impl<'de> de::Deserialize<'de> for TipSetBundle {
    ...
    where
        D: Deserializer<'de>,
    {
        let cm = CborTipsetBundle::deserialize(deserializer)?;
        Ok(TipSetBundle {
            blocks: cm.0,
            secp_msgs: cm.1,
            secp_msg_includes: cm.2,
            bls_msgs: cm.3,
            bls_msg_includes: cm.4,
        })
    }
}
    ...
    C: AsyncRead + AsyncWrite,
{
    ...
    fn upgrade_outbound(self, mut socket: Negotiated<C>, info: Self::Info) -> Self::Future {
        ...
        println!(
            "Wrote this number of bytes on blocksync: {:?}",
        ...
        println!("BYTESSS: {:?}", buf.len());
        let decoded: Response = from_slice(&buf).unwrap();
        //        socket.read_buf(&mut buf).and_then(move |size| {
        //            println!("Read this: {:?}", buf);
        //            println!("With size {:?}", size);
        //            Ok(())
        //        });
        //        read(socket, buf).map_err(|e| {
        //            println!("IO error {:?}", e);
        //        }).map(move |(reader, _in_buf, bytes_read)| {
        //            println!("Got {} bytes", bytes_read);
        //            let quit = bytes_read > 0; // quit when we got *some* bytes
        //            (reader, writer, quit)
        //        });
        //        println!("READ: {:?}", buf);
        future::ok(())
